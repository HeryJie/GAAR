...
class Supervision_Train(pl.LightningModule):
    ...
    def forward(self, x):
        # only net is used in the prediction/inferenc e
        if self.training:
            seg_pre = []
            out, au, r_loss = self.net(x, self.current_step)
            seg_pre.append(out)
            seg_pre.append(au)
            return seg_pre, r_loss
        else:
            seg_pre = self.net(x, self.current_step)
            return seg_pre

    def training_step(self, batch, batch_idx):
        img, mask = batch['img'], batch['gt_semantic_seg']
        self.current_step += 1
        prediction, r_loss = self.forward(img)
        loss = self.loss(prediction, mask) + r_loss
